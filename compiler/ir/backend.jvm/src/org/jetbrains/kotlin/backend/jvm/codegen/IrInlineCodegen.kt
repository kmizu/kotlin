...
class IrInlineCodegen(
    ...
    private fun IrFunction.originalDescriptor() =
        if (isSuspend) suspendFunctionOriginal().descriptor else descriptor
    ...
    override fun genCall(
        ...
        // Calls to suspend functions are remapped to $$forInline versions by IrSourceCompilerForInline,
        // so to correctly detect cycles, we need to normalize descriptors.
        val context = codegen.irFunction.originalDescriptor()
        ...
        if (!state.globalInlineContext.enterIntoInlining(context, expression.symbol.owner.originalDescriptor(), element)) {
        ...
        try {
            ...
            state.globalInlineContext.exitFromInlining(context)
        }
    }
